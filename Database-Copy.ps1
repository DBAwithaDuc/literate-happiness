...
$SQLDrift="CORPDB1720.CORP.SAAB.SE,11433"
$date = Get-Date -format "yyyyMMddHHmm"
$fname="databasecopy"
$filename =$fname+"_"+$date+".log"
$path = Get-Location 
$path =$path.Path
#########################  Functions ###################################################
...

{
    ...
    try {
        $message = copy-dbadatabase -source $sqlinstance1 -destination $sqlinstance2 -database $database -newname $Newdbname -backupRestore -SharedPath $Netshare  -withReplace -Force <# Add -UseLastBackup if you want to use lastbackup #>
    }
    catch {
        $ErrorMessage = $_.Exception.Message
        $FailedItem = $_.Exception.ItemName
        "$message,$FailedItem, $ErrorMessage" | out-file $path+$filename -append
    }
