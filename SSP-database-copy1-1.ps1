...
$databases = Import-Csv -Path C:\scripts\SESP_databases3PROD.csv
...
$logfile = 'C:\scripts\SSPDB16892.txt'
...

{
    ...
    copy-dbadatabase -source $sqlinstance1 -destination $sqlinstance2 -database $database -newname $newname -backupRestore -SharedPath $Netshare  -withReplace -Force -| Write-output <# Add -UseLastBackup if you want to use lastbackup #>
...
function DBFileSize # Checks if and database fits on the target server #
{
  ...
  $logdisk1 = Get-DbaDbFile -SqlInstance $sqlinstance1 -Database $database |  Where-Object typedescription -eq log | Select-Object size
    ...
    $datasize2 = get-freespace -sqlinstance $sqlinstance2 -file 'data'
    $logsize2 = get-freespace -sqlinstance $sqlinstance2 -file 'LOG'
