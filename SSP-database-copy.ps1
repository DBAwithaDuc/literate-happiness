...
$databases = Get-Content -Path C:\scripts\SESP_databases3PROD.txt
...
{
    param($sqlinstance1, $database, $sqlinstance2, $database, $Netshare)

    Write-Host 'Starting the database copying'
    copy-dbadatabase -source $sqlinstance1 -destination $sqlinstance2 -database $database -newname $database -backupRestore -SharedPath $Netshare  -withReplace | Write-output <# Add -UseLastBackup if you want to use lastbackup #>

    <# Set owner of the database to SA #>
    Write-Host 'Set DBowner for ' $database ' to SA'
    set-dbadbowner -sqlinstance $sqlinstance2 -database $database  | Write-output <# Sets DBowner for $database to SA #>
}
